import pkg from 'pg';
const { Pool } = pkg;
import fs from 'fs';

const NEON_CONNECTION_STRING = "postgresql://neondb_owner:npg_MQYpxwa17zRV@ep-jolly-grass-b3c2d7vr-pooler.c-4.ap-southeast-1.aws.neon.tech/neondb?sslmode=require";

const pool = new Pool({
    connectionString: NEON_CONNECTION_STRING,
    ssl: { rejectUnauthorized: false }
});

async function main() {
    const client = await pool.connect();
    console.log("Connected to Neon PostgreSQL!");

    try {
        console.log("Ensuring database tables exist...");
        await client.query(`
            CREATE TABLE IF NOT EXISTS workspaces (
                id VARCHAR(255) PRIMARY KEY,
                profit_percent NUMERIC DEFAULT 5,
                cloud_url TEXT,
                last_synced BIGINT,
                created_at BIGINT
            );

            CREATE TABLE IF NOT EXISTS accounts (
                id VARCHAR(255) PRIMARY KEY,
                workspace_id VARCHAR(255) NOT NULL,
                name VARCHAR(255) NOT NULL,
                type VARCHAR(50) NOT NULL,
                balance NUMERIC DEFAULT 0,
                limit_val NUMERIC DEFAULT 0,
                initial_balance NUMERIC DEFAULT 0
            );

            CREATE TABLE IF NOT EXISTS categories (
                id VARCHAR(255) PRIMARY KEY,
                workspace_id VARCHAR(255) NOT NULL,
                label VARCHAR(255) NOT NULL,
                icon VARCHAR(255)
            );

            CREATE TABLE IF NOT EXISTS suppliers (
                id VARCHAR(255) PRIMARY KEY,
                workspace_id VARCHAR(255) NOT NULL,
                name VARCHAR(255) NOT NULL
            );

            CREATE TABLE IF NOT EXISTS transactions (
                id VARCHAR(255) PRIMARY KEY,
                workspace_id VARCHAR(255) NOT NULL,
                date BIGINT NOT NULL,
                amount NUMERIC NOT NULL,
                type VARCHAR(50) NOT NULL,
                description TEXT,
                source_account_id VARCHAR(255),
                destination_account_id VARCHAR(255),
                income_source VARCHAR(50),
                expense_category VARCHAR(255),
                supplier_id VARCHAR(255),
                is_profit_withdrawal BOOLEAN DEFAULT FALSE,
                tags TEXT[],
                notes TEXT,
                invoice_url TEXT,
                payment_proof_url TEXT,
                created_at BIGINT
            );
        `);
        console.log("Schema ready!");

        const workspaceId = "SHIVRAT";

        // Read workspace_dump.json if available
        if (fs.existsSync('./workspace_dump.json')) {
            const wsDump = JSON.parse(fs.readFileSync('./workspace_dump.json', 'utf8'));
            console.log("Found workspace_dump.json");

            await client.query(`
                INSERT INTO workspaces (id, profit_percent, cloud_url, last_synced)
                VALUES ($1, $2, $3, $4)
                ON CONFLICT (id) DO UPDATE 
                SET profit_percent = EXCLUDED.profit_percent, cloud_url = EXCLUDED.cloud_url, last_synced = EXCLUDED.last_synced;
            `, [workspaceId, wsDump.profitPercent || 5, wsDump.cloudUrl || '', wsDump.lastSynced || Date.now()]);

            if (Array.isArray(wsDump.accounts)) {
                console.log(`Inserting ${wsDump.accounts.length} accounts...`);
                for (const acc of wsDump.accounts) {
                    await client.query(`
                        INSERT INTO accounts (id, workspace_id, name, type, balance, limit_val)
                        VALUES ($1, $2, $3, $4, $5, $6)
                        ON CONFLICT (id) DO UPDATE
                        SET name = EXCLUDED.name, type = EXCLUDED.type, balance = EXCLUDED.balance, limit_val = EXCLUDED.limit_val;
                    `, [acc.id, workspaceId, acc.name, acc.type, acc.balance || 0, acc.limit || 0]);
                }
            }

            if (Array.isArray(wsDump.categories)) {
                console.log(`Inserting ${wsDump.categories.length} categories...`);
                for (const cat of wsDump.categories) {
                    await client.query(`
                        INSERT INTO categories (id, workspace_id, label, icon)
                        VALUES ($1, $2, $3, $4)
                        ON CONFLICT (id) DO UPDATE SET label = EXCLUDED.label, icon = EXCLUDED.icon;
                    `, [cat.id, workspaceId, cat.label, cat.icon || null]);
                }
            }

            if (Array.isArray(wsDump.suppliers)) {
                console.log(`Inserting ${wsDump.suppliers.length} suppliers...`);
                for (const sup of wsDump.suppliers) {
                    await client.query(`
                        INSERT INTO suppliers (id, workspace_id, name)
                        VALUES ($1, $2, $3)
                        ON CONFLICT (id) DO UPDATE SET name = EXCLUDED.name;
                    `, [sup.id, workspaceId, sup.name]);
                }
            }
        }

        // Read all_transactions.json
        const txFile = fs.existsSync('./all_transactions.json') ? './all_transactions.json' : './transactions_dump.json';
        if (fs.existsSync(txFile)) {
            console.log(`Reading transactions from '${txFile}'...`);
            const txs = JSON.parse(fs.readFileSync(txFile, 'utf8'));
            console.log(`Loaded ${txs.length} transactions from JSON dump.`);

            const chunkSize = 100;
            for (let i = 0; i < txs.length; i += chunkSize) {
                const chunk = txs.slice(i, i + chunkSize);
                const valueRows = [];
                const queryParams = [];
                let pIdx = 1;

                for (const tx of chunk) {
                    const txId = tx.id || `tx_${Math.random().toString(36).substring(2)}`;

                    valueRows.push(`($${pIdx++}, $${pIdx++}, $${pIdx++}, $${pIdx++}, $${pIdx++}, $${pIdx++}, $${pIdx++}, $${pIdx++}, $${pIdx++}, $${pIdx++}, $${pIdx++}, $${pIdx++}, $${pIdx++}, $${pIdx++}, $${pIdx++}, $${pIdx++}, $${pIdx++})`);

                    queryParams.push(
                        txId, workspaceId, Number(tx.date) || Date.now(), Number(tx.amount) || 0, tx.type || 'EXPENSE',
                        tx.description || '', tx.sourceAccountId || null, tx.destinationAccountId || null,
                        tx.incomeSource || null, tx.expenseCategory || null, tx.supplierId || null,
                        !!tx.isProfitWithdrawal, Array.isArray(tx.tags) ? tx.tags : [], tx.notes || null,
                        tx.invoiceUrl || null, tx.paymentProofUrl || null, Number(tx.createdAt || tx.date) || Date.now()
                    );
                }

                const batchQuery = `
                    INSERT INTO transactions (
                        id, workspace_id, date, amount, type, description, 
                        source_account_id, destination_account_id, income_source, 
                        expense_category, supplier_id, is_profit_withdrawal, 
                        tags, notes, invoice_url, payment_proof_url, created_at
                    ) VALUES ${valueRows.join(', ')}
                    ON CONFLICT (id) DO UPDATE SET
                        date = EXCLUDED.date, amount = EXCLUDED.amount, type = EXCLUDED.type,
                        description = EXCLUDED.description, source_account_id = EXCLUDED.source_account_id,
                        destination_account_id = EXCLUDED.destination_account_id, income_source = EXCLUDED.income_source,
                        expense_category = EXCLUDED.expense_category, supplier_id = EXCLUDED.supplier_id,
                        is_profit_withdrawal = EXCLUDED.is_profit_withdrawal, tags = EXCLUDED.tags,
                        notes = EXCLUDED.notes, invoice_url = EXCLUDED.invoice_url,
                        payment_proof_url = EXCLUDED.payment_proof_url, created_at = EXCLUDED.created_at;
                `;

                await client.query(batchQuery, queryParams);
                console.log(`Inserted ${Math.min(i + chunkSize, txs.length)} / ${txs.length} transactions`);
            }
        }

        const countRes = await client.query("SELECT COUNT(*) FROM transactions WHERE workspace_id = $1;", [workspaceId]);
        console.log(`🎉 SUCCESS: ${countRes.rows[0].count} transactions migrated to Neon Postgres!`);

    } catch (err) {
        console.error("Local Dump Migration Error:", err);
    } finally {
        client.release();
        await pool.end();
        process.exit(0);
    }
}

main();
